    ...
    switch (header) {
      ...
      case 'TimingPoints':
        ...
        for (var i = 0; i < lines.length; i++) {
          lines[i].replace(/^(.*),(.*),(.*),(.*),(.*),(.*),(.*),(.*)/gmi,
          function(_, Offset, MillisecondsPerBeat, Meter, SampleType,
            SampleSet, Volume, Inherited, KiaiMode) {
            timingPoints.push({
              'Offset': +Offset,
      ...
      case 'Colours':
        ...
        var colors = [];
        for (var i = 0; i < lines.length; i++) {
          lines[i].replace(/^.* : ([0-9]*),([0-9]*),([0-9]*)/, function(_,
            r, g, b) {
            colors.push('#' + ((1 << 24) + (+r << 16) + (+g << 8) + +b)
              .toString(16).slice(1));
          });
        }
      ...
      case 'HitObjects':
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (/^([0-9-]*),([0-9-]*),([0-9-]*),(1|5),([0-9-]*)/.test(lines[i])) {
            ...
            lines[i].replace(
              /^([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*)/,
              function(_, x, y, time, type, hitSound) {
                circle = {
                  'x': +x,
                  'y': +y,
                  'time': +time,
            ...
            var slider = {};
            lines[i].replace(
              /^([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*),([CBLP](\|[0-9-]*:[0-9-]*)*),([0-9-]*),/,
              function(_, x, y, time, type, hitSound, properties, __,
                repeat) {
                slider = {
                  'time': +time,
                  'type': +type,
                ...
                for (var j = 0; j < arr.length; j++) {
                  arr[j].replace(/^([0-9-]*):([0-9-]*)$/, function(_, cX,
                    cY) {
                    var point = {
                      'x': +cX,
                      'y': +cY
            ...
            /^([0-9-]*),([0-9-]*),([0-9-]*),(8|12),([0-9-]*),([0-9-]*)/.test(
              lines[i])) {
            var spinner = {};
            lines[i].replace(
              /^([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*),([0-9-]*)/,
              function(_, x, y, time, type, hitSound, endTime) {
                spinner = {
                  'x': +x,
                  'y': +y,
                  'time': +time,
